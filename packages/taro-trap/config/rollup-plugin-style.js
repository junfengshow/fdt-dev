...
const staticLink = 'dist/styles/components';
const reg = /components\/([\s\S]*)\//;

// 获取demo所有文件
const getDemoPath = () => {
  const packagePaths = globby.sync(`${cwd}/src/components/*/*.less`);
...
const writeDemoFile = res => {
  ...
  res.packagePaths.map(val => {
    ...
    fs.copyFileSync(val, `${cwd}/${staticLink}/${newFile}`);
  });
};
...
export default function RollupPluginDemo({ watch }) {
  return {
    ...
    buildStart(source) {
      if (watch) {
        nodeWatch(path.join(cwd, '/src/style'), { recursive: true }, function(
          evt,
          name,
        ) {
          console.log('======= watch ======');
