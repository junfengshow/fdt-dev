...
async function runBuild() {
  try {
    ...
    let entryDataStr = await fsPromise.readFile(
      path.resolve(__dirname, 'entry.template.tsx'),
    );
    let routesStr = await fsPromise.readFile(
      path.resolve(__dirname, './project.routes.js'),
    );

    routesStr = formatRoutes(routesStr.toString());

    await fsPromise.writeFile(
      joinPath(`src/${localDirName}/routes.js`),
      routesStr,
    );

    await fsPromise.writeFile(
      joinPath(`src/${localDirName}/${entryName}`),
      entryDataStr,
    );
  } catch (e) {
    // 总体就出错了
